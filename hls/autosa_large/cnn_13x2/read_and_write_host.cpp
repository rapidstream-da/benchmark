...
//  data_t A[I][K], B[K][J], C[I][J], C_golden[I][J]; 
...

  {
    ...
    float *dev_B_unserialized = (float *)malloc((128) * (256) * sizeof(float));
    float *dev_B = (float *)malloc(32768 * sizeof(float));
    float *dev_C_unserialized = (float *)malloc((208) * (128) * sizeof(float));
    float *dev_C = (float *)malloc(26624 * sizeof(float));

    // Initialize host buffers
    memcpy(dev_A_unserialized, A, (208) * (256) * sizeof(float));
    memcpy(dev_B_unserialized, B, (128) * (256) * sizeof(float));
    memcpy(dev_C_unserialized, C, (208) * (128) * sizeof(float));
    ...
    for (int i = 0; i < 1; i++) {
        B_t16 *buffer_B_tmp = (B_t16 *)malloc((32768) * sizeof(float));
        buffer_B.push_back(buffer_B_tmp);
    }
    for (int i = 0; i < 1; i++) {
        C_t16 *buffer_C_tmp = (C_t16 *)malloc((26624) * sizeof(float));
    ...
    for (int i = 0; i < 1; i++) {
        memcpy(buffer_B[i], dev_B, (32768) * sizeof(float));
    }

    {
      // Launch the kernel
      top(buffer_A[0], buffer_B[0], buffer_C[0]);
    }
    for (int i = 0; i < 1; i++) {
        memcpy(dev_C, buffer_C[i], (26624) * sizeof(float));
    }

    host_deserialize_C(dev_C_unserialized, dev_C);
    // Restore data from host buffers
    memcpy(C, dev_C_unserialized, (208) * (128) * sizeof(float));
